#!/bin/bash
#Rebechecker v4.2, Date: 04.08.2018
time=`date +"%H:%M"`

#parameters:
domain="47.91.95.44"
tag="donaurebe" 				#pushbullet channel name
min="50"						#minimum price to trigger expensive push
path="/home/pi/donau" 	#folder for temp files. Favour RAM Storage for 24/7 use
...
do
	...
	curl -k https://$domain/index/show.html?page=$page > $path/source$page.json
	jq '.data[] | "\(.listingname) \(.price)"' $path/source$page.json | awk -F '"' '{print $2}' >> $path/plu$state.txt
	sleep 1.5
done
#price
echo downloading price json
curl -k https://$domain/index/fc.html?cond=price > $path/sourcep.json
sleep 1.5
